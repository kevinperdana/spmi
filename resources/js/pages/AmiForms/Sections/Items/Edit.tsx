...
const mapCapaianUnit = (value: string) => {
    if (value === 'dokumen_tersedia' || value === 'angka') return value;
    if (value === 'tersedia') return 'dokumen_tersedia';
    return 'angka';
};

export default function Edit({ form, section, item, satuanOptions, targetOptions, capaianOptions }: Props) {
    ...
    const submit: FormEventHandler = (event) => {
        ...
        put(`/ami-forms/${form.id}/sections/${section.id}/items/${item.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Item - ${section.title}`} />
